...
# Changed output file name for clarity
OUTPUT_CSV_FILE = os.path.join('..', 'data', 'processed', 'manual_checkout_list_no_eslint.csv')
...
def get_bug_commits_for_repo(repo_full_name, target_count):
    ...
    while len(bug_commit_shas) < target_count:
        try:
            ...
            if page > 100:
                print(f"Warning: Searched 10,000 commits for {repo_full_name} and didn't find {target_count} bug fixes. Moving on.")
                break
...
# 2. Filter for has_eslint == False
# Note: We do NOT check 'analysis_status' here because these repos don't have config analysis.
if 'has_eslint' not in df_all_repos.columns:
    print(f"Error: Column 'has_eslint' not found in {INPUT_CSV_FILE}")
    exit(1)

# Filter for repos that DO NOT have ESLint
df_no_eslint_repos = df_all_repos[df_all_repos['has_eslint'] == False]
print(f"Found {len(df_no_eslint_repos)} repositories WITHOUT ESLint (out of {len(df_all_repos)} total).")

# 3. Sample repos
actual_sample_size = min(REPOS_TO_SAMPLE, len(df_no_eslint_repos))
if actual_sample_size < REPOS_TO_SAMPLE:
    print(f"Warning: Only found {actual_sample_size} 'No ESLint' repos to sample from.")

print(f"Randomly sampling {actual_sample_size} repos from the 'No ESLint' group...")
df_sample = df_no_eslint_repos.sample(n=actual_sample_size, random_state=42)  # Reproducible sample
